...
fun UpdateDialog(
    ...
    when (status) {
        is UpdateStatus.NewVersion -> {
            AlertDialog(
                onDismissRequest = { vm.clearUpdateStatus() },
                ...
                dismissButton = {
                    TextButton(onClick = { vm.clearUpdateStatus() }) {
                        Text("Not Now")
                    }
                }
            )
        ...
        is UpdateStatus.Error -> {
            if (status.isManual) {
                ...
                // Silent check error, just clear it
                LaunchedEffect(status) {
                    vm.clearUpdateStatus()
                }
            }
        ...
        UpdateStatus.Checking -> {
            // Optional: show a loading indicator for manual checks
        }
        UpdateStatus.Idle -> {}
    }
}
