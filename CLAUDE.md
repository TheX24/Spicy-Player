...
- `PORTING_BLUEPRINT_LYRICS.md` (project root) is the user's own reverse-engineering of the whole lyrics engine (clock, springs, splines, CSS cascade semantics, scroll/virtualizer) — treat as a primary spec alongside `spicy-lyrics/` source, especially when it and a literal source read disagree (ask the user, don't guess).
- **Compose gotcha**: `drawText(textLayoutResult, brush = …)` evaluates the brush in text-LOCAL coordinates (canvas is pre-translated to the text's topLeft) — gradient endpoints must be `0f..width`, not canvas-absolute `xPos..xPos+width`. Got this wrong once in `LyricsRenderer.kt` and cost several device-test rounds to diagnose.
- Romanization deps: use `com.atilika.kuromoji:kuromoji-ipadic` (Japanese) and `com.belerweb:pinyin4j` (Chinese) — NOT `tinypinyin`, which isn't on Maven Central. Kuromoji's jar needs `packaging { resources.excludes += "/META-INF/{CONTRIBUTORS,LICENSE,NOTICE,README}.md" }` in `app/build.gradle.kts` to avoid resource merge conflicts.
